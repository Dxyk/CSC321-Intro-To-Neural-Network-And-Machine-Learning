...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        D_X_preds = D_X(images_X)
        real_labels = Variable(torch.ones(labels_X.size(0)))
        D_X_loss = F.mse_loss(D_X_preds, real_labels)

        D_Y_preds = D_Y(images_Y)
        real_labels = Variable(torch.ones(labels_Y.size(0)))
        D_Y_loss = F.mse_loss(D_Y_preds, real_labels)
        ...
        D_fake_X = D_X(fake_X)
        # batch_size
        fake_labels = Variable(torch.zeros(fake_X.size(0)))
        D_X_loss = F.mse_loss(D_fake_X, fake_labels)
        ...
        D_fake_Y = D_Y(fake_Y)
        # batch_size
        fake_labels = Variable(torch.zeros(fake_Y.size(0)))
        D_Y_loss = F.mse_loss(D_fake_Y, fake_labels)
        ...
        D_fake_X = D_X(fake_X)
        fake_labels = Variable(torch.ones(fake_X.size(0)))
        g_loss = F.mse_loss(D_fake_X, fake_labels)
        ...
        D_fake_Y = D_Y(fake_Y)
        fake_labels = Variable(torch.ones(fake_Y.size(0)))
        g_loss = F.mse_loss(D_fake_Y, fake_labels)
